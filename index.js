...
async function fetchCandidateTokens() {
  ...
  // Source 1: Dexscreener - Solana trending pairs (best source)
  try {
    ...
    console.log('S1 dex pairs/solana:', results.length);
  ...
  // Source 5: Pump.fun top market cap
  try {
  ...
  // Source 6: DexScreener search - pump
  try {
    const res = await axios.get('https://api.dexscreener.com/latest/dex/search?q=pump', { timeout: 10000 });
    (res.data?.pairs||[]).filter(p => p.chainId==='solana').forEach(p => {
      if(p.baseToken?.address) results.push(p.baseToken.address);
    });
    console.log('S6 dex search pump:', results.length);
...
async function analyzeForSignal(ca) {
  try {
    ...
    // Not dead token (some volume)
    if ((p.volume?.h24 || 0) < 100) return null;

    // Log what we find for debugging
    console.log('TOKEN:', p.baseToken?.symbol, '| Age:', ageH.toFixed(1)+'h', '| MC:', fmt(mc), '| Liq:', fmt(liq));

    // Rug check
    const rug = await getRugcheck(ca);
    if (rug?.score && rug.score > MAX_RUG_SCORE) return null;

    // Holder concentration — reject if top 10 > 50%
    if (rug) {
      const holders = rug?.topHolders || rug?.holders || [];
      if (holders.length > 0) {
...
async function runScan(manual) {
  ...
  isScanning = true;
  if (manual) bot.sendMessage(ADMIN_ID, '🔍 Scanning... fetching from 6 sources\n⚡ DEBUG: Minimal filters — finding all tokens');
  ...
  try {
    ...
    if (manual) {
      bot.sendMessage(ADMIN_ID, found === 0
        ? '🔍 Done — checked ' + checked + ' tokens, none passed score filter (' + SCORE_PASS_MIN + '/100).\n\nNext auto-scan in 15 mins.'
...
bot.sendMessage(ADMIN_ID,
  '🚀 *Bot Started! v3 DEBUG*\n\n' +
  '⏰ Filters: Liq $1K+ | Vol $100+\n' +
  '🔄 Scan: every 15 mins\n' +
