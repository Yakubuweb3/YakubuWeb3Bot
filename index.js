...
async function detectBundlesAndSnipers(pairAddress, tokenMint, pairCreatedAtMs) {
  try {
    ...
    for (const sig of window) {
      ...
      if (distinctBuyers.length >= 2) {
        bundleTxCount++;
        distinctBuyers.forEach(w => bundledWallets.add(w));
      } else if (distinctBuyers.length === 1) {
        const secsAfterLaunch = ((sig.blockTime * 1000) - pairCreatedAtMs) / 1000;
        if (secsAfterLaunch >= 0 && secsAfterLaunch <= SNIPER_WINDOW_SECONDS) {
          sniperWallets.add(distinctBuyers[0]);
        }
      }
      await new Promise(r => setTimeout(r, 300));
    }

    sniperWallets.forEach(w => { if (bundledWallets.has(w)) sniperWallets.delete(w); });
...
async function formatMsg(p, ca, chatId) {
  ...
  // NEW: Real on-chain bundle/sniper detection — overrides the Rugcheck estimate above when available
  if (p.pairAddress && p.baseToken?.address && p.pairCreatedAt) {
    const onchain = await detectBundlesAndSnipers(p.pairAddress, p.baseToken.address, p.pairCreatedAt);
    if (onchain.available) {
      bundleInfo = onchain.bundleCount > 0
        ? onchain.bundleCount + ' wallets bundled (' + onchain.bundleTxCount + ' tx) — hold ' + onchain.bundleCurrentPct.toFixed(1) + '% now' + (onchain.bundleCurrentPct > 15 ? ' ⚠️' : '')
        : 'No bundles found (on-chain scan, ' + onchain.txScanned + ' early tx checked)';
      sniperInfo = onchain.sniperCount > 0
        ? onchain.sniperCount + ' snipers — hold ' + onchain.sniperCurrentPct.toFixed(1) + '% now'
        : 'No snipers found (on-chain scan, ' + onchain.txScanned + ' early tx checked)';
    }
  }
...
async function sendAnalysis(chatId, ca, username) {
  const loadMsg = await bot.sendMessage(chatId, 'Analyzing token (deep bundle/sniper scan — may take 30-60s)...');
  try {
    const p = await getTokenData(ca);
