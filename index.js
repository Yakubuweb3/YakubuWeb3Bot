...
const MAX_AGE_HOURS = 5;
const MAX_RUG_SCORE = 500;
...
async function analyzeForSignal(ca) {
  try {
    ...
    if (!dex.includes('pump') && !dex.includes('raydium')) return null;
    ...
    if (ch1 > 15) return null;
    if (ch5m <= 0 && ch15m <= 0) return null;

    const vol24 = p.volume?.h24 || 0;
    if (vol24 < 5000) return null;

    const buys = p.txns?.h24?.buys || 0;
    const sells = p.txns?.h24?.sells || 0;
    if (buys < sells * 0.8) return null;
