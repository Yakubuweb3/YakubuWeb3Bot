...
function closePaperTrade(id, result, pnlPct, pnlSol) {
  try { db.prepare('UPDATE paper_trades SET status = ?, result = ?, pnl_pct = ?, pnl_sol = ?, closed_at = ? WHERE id = ?').run('closed', result, pnlPct, pnlSol, Date.now(), id); }
  catch(e) {}
}
...
async function sendSignalForReview(ca, existingP, existingRug, isAutoScan) {
  ...
  } catch(e) { console.error('Signal review error:', e.message); if (!isAutoScan) bot.sendMessage(ADMIN_ID, 'Error preparing signal!'); }
}
