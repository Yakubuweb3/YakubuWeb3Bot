...
async function fetchCandidateTokens() {
  ...
  try {
    ...
    (res.data?.pairs || []).forEach(t => { if (t.baseToken?.address) results.push(t.baseToken.address); });
  } catch(e) {}
    ...
    if (rug) {
      ...
      if (Array.isArray(holdersData) && holdersData.length > 0) {
        ...
        if (list.length > 0) {
          ...
          if (top10pct > 40) return null;
        }
      }
      if (rug.insiderNetworks && rug.insiderNetworks.length > 20) return null;
      if (rug.snipers && rug.snipers.length > 20) return null;
    }

    // Road Map 2: DEX strength scoring
...
bot.onText(/\/help/, (msg) => {
  ...
  bot.sendMessage(msg.chat.id, '📋 *Commands:*\n\nSend CA — Analyze\n/pnl <CA> — PNL Card\n/price <symbol> — Price\n/status' + adminCmds, { parse_mode: 'Markdown' });
});
...
bot.on('message', async (msg) => {
  ...
  const ca = text.trim();
  if (ca.length >= 32 && ca.length <= 50 && /^[A-Za-z0-9]+$/.test(ca)) await sendAnalysis(chatId, ca, msg.from?.username);
});
