...
// DEX PAID - Original working method
...
// HOLDER COUNT - Multiple fallbacks
async function getHolderCount(ca, rugData) {
  // 1. Try rugcheck tokenMeta first
  ...
  try {
    ...
    if (res.data?.total) {
      return res.data.total.toLocaleString();
    }
  } catch(e) {}

  // 3. Try rugcheck holders endpoint
  try {
    const res = await axios.get(
      `https://api.rugcheck.xyz/v1/tokens/${ca}/holders`,
      { timeout: 5000 }
    );
    if (res.data?.total) return res.data.total.toLocaleString();
...
async function formatMsg(p, ca, chatId) {
  ...
  // DEX PAID - using original working method
