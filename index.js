...
function isLpOrProgram(address) {
  // Filter out known LP/program addresses
  const known = [
  ...
  return known.includes(address) || address.length < 30;
}
...
async function formatMsg(p, ca) {
  ...
  if (rug) {
    ...
    // HOLDERS — Filter LP tokens
    if (rug.topHolders && rug.topHolders.length > 0) {
      const filtered = rug.topHolders.filter(h => !isLpOrProgram(h.address));
      ...
      const t10 = top10.reduce((a, b) => a + (b.pct || 0), 0);
      const t20 = top20.reduce((a, b) => a + (b.pct || 0), 0);
      ...
      if (rug.creator) {
        ...
        devHolding = devH ? devH.pct?.toFixed(1) + '%' : '0%';
      }
    }
    ...
    if (rug.insiderNetworks && rug.insiderNetworks.length > 0) {
      bundleCount = rug.insiderNetworks.length;
      const totalPct = rug.insiderNetworks.reduce(
        (a, b) => a + (b.holdingPercent || 0), 0
      );
      const nowPct = rug.insiderNetworks.reduce(
        (a, b) => a + (b.currentHoldingPercent || 0), 0
      );
      bundlePct = totalPct.toFixed(1) + '%';
      bundleInfo = `${bundleCount} bundles • ${bundlePct} bought • ${nowPct.toFixed(1)}% now`;
    }

    // SNIPERS
    if (rug.snipers && rug.snipers.length > 0) {
      sniperCount = rug.snipers.length;
      const sPct = rug.snipers.reduce(
        (a, b) => a + (b.holdingPercent || 0), 0
      );
      const sNow = rug.snipers.reduce(
        (a, b) => a + (b.currentHoldingPercent || 0), 0
      );
      sniperPct = sPct.toFixed(1) + '%';
      sniperInfo = `${sniperCount} snipers • ${sniperPct} bought • ${sNow.toFixed(1)}% now`;
    }
