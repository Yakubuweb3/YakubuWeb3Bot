...
async function getHolderCount(ca) {
  try {
    const res = await axios.get(
      `https://api.rugcheck.xyz/v1/tokens/${ca}/holders/count`
    );
    if (res.data && res.data.count) return res.data.count;
  } catch(e) {}
  
  // Fallback — try Solscan
  try {
    const res = await axios.get(
      `https://public-api.solscan.io/token/holders?tokenAddress=${ca}&limit=1&offset=0`,
...
// CHECK DEX PAID — proper method
async function checkDexPaid(ca) {
  try {
    // Check DexScreener directly
    const res = await axios.get(
    ...
    if (Array.isArray(orders) && orders.length > 0) {
      const paid = orders.some(o =>
        o.status === 'approved' &&
        (o.type === 'tokenProfile' || o.type === 'communityTakeover' ||
         o.type === 'trendingBarAd' || o.type === 'bannerAd')
      );
...
async function formatMsg(p, ca, chatId) {
  ...
  // DEX PAID — proper check
  const dexPaid = await checkDexPaid(ca);

  // RUGCHECK + HOLDER COUNT in parallel
  const [rug, holderCountReal] = await Promise.all([
    getRugcheck(ca),
    getHolderCount(ca)
  ]);
  ...
  let risks = '✅ Clean';
  let devInfo = 'N/A';
  let holdersCount = holderCountReal
    ? holderCountReal.toLocaleString()
    : 'N/A';
  let top10 = 'N/A';
  let top20 = 'N/A';
  ...
  if (rug) {
    ...
    if (Array.isArray(holdersData) && holdersData.length > 0) {
      ...
      if (list.length > 0) {
        // Use real holder count if available
        if (!holderCountReal) {
