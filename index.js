...
// SAVE — allows multiple entries per CA
...
// GET FIRST ENTRY — always returns first time CA was analyzed
...
// HOLDERS — multiple fallbacks
async function getHolderCount(ca, rugData) {
  // 1. Rugcheck tokenMeta
  if (rugData?.tokenMeta?.holderCount) {
    return rugData.tokenMeta.holderCount.toLocaleString();
  }

  // 2. Rugcheck markets
  if (rugData?.markets?.length > 0) {
    for (const m of rugData.markets) {
      if (m.holderCount && m.holderCount > 0) {
        return m.holderCount.toLocaleString();
      }
    }
  }

  // 3. Rugcheck lp data
  if (rugData?.holders?.length > 0) {
    return rugData.holders.length.toLocaleString() + '+';
  }
  ...
  try {
    ...
    if (res.data?.data?.holder && res.data.data.holder > 0) {
      return res.data.data.holder.toLocaleString();
    }
  } catch(e) {}
  ...
  try {
    ...
    if (res.data?.data?.total) {
      return res.data.data.total.toLocaleString();
    }
  } catch(e) {}

  // 6. Rugcheck topHolders length
  if (rugData?.topHolders?.length) {
    return rugData.topHolders.length + '+';
  }
...
async function formatMsg(p, ca, chatId) {
  ...
  if (rug) {
    ...
    riskLevel = riskScore >= 800 ? '🔴 HIGH'
      : riskScore >= 500 ? '🟡 MEDIUM' : '🟢 LOW';
    ...
    if (rug.creator) {
      const devBal = rug.creatorBalance
        ...
        : '0 SOL';
      const devH = (rug?.topHolders || [])
        .find(h => h.address === rug.creator);
